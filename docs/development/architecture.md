...
操作対応済みの既知カテゴリ（light / plug / bot / curtain / humidifier / lock / aircon / ir_light）に
マップできない種別と操作未対応の赤外線デバイス（`infraredRemoteList` のうちエアコン・ライト以外）
の扱いは、次の 2 系統に振り分ける（一覧に「未対応」バッジを乱立させない）。

- **センサー系（温湿度計 Meter・人感センサー Motion Sensor）は、`list_devices` で一覧から除外**
  し（`is_sensor` で判定）、センサー画面（`get_sensors`）に一本化する。従来は一覧に `other`
  として重複表示していたが、二重表示を解消するため一覧には出さない。`build_sensor_readings` が
  status body のフィールド有無で計測を積み分け、数値（温度/湿度/電池）は gauge、状態
  （人感 `moveDetected` / 明るさ `brightness`）は state 表現にする。並べ替え・永続化は deviceId
  ベースで種別非依存（詳細は [switchbot-api.md](./switchbot-api.md) 参照）。
- **操作もセンサー読み取りもできない「その他」（ハブ類・操作未対応の赤外線リモコン等）は、
  一覧末尾の折りたたみ「その他」セクションに集約**する（`OtherDevicesSection`。radix
  `Collapsible`・既定は折りたたみで展開状態は永続化しない）。各行は読み取り専用
  （`OtherDeviceRow`：アイコン＋名前＋モデル名の muted 表示）で、操作要素・ピン留め・
  カード単位の「未対応」バッジは持たない。フロントの振り分けは `isOtherDevice(device)`
  （= `deviceInteraction(device) === "none"`）の単純述語で行う。

**赤外線エアコン（Air Conditioner）は `aircon` カテゴリで操作対応**する。赤外線は status を
持たないため、`list_devices` の status 取得対象から除外し（`meta.supported && !meta.infrared`）、
`build_device` は初期値（power off / 26℃ / cool / auto）を返す。表示・操作の唯一のソースは
**「最後に送信した値」をローカル（plugin-store）に永続保持**したもので、device-store が
refresh 時に controls へ重畳する。操作は詳細画面で温度・モード・風量・電源を選び、常に
`setAll`（全状態一括送信）する。enum→数値エンコードと parameter 組み立ては Rust
`mapping.rs`（`aircon_parameter`）が所有し、フロントは意味論（"cool"/"high"）だけを扱う
（決定1。詳細は [switchbot-api.md](./switchbot-api.md) 参照）。

**赤外線ライト（Light / DIY Light）は `ir_light` カテゴリで操作対応**する。エアコンと同じく status を
持たないため status 取得対象から除外し、`build_device` は電源 off の初期値を返す。ライトは絶対的な
明るさ・状態を持たず、電源（turnOn/turnOff）と相対的な明暗（brightnessUp/brightnessDown）のみを扱う。
device-store の `operateIrLight` が、電源（on/off）は楽観更新 + `irLightStates` へ永続化 + 失敗時ロールバック、
明暗（brighter/dimmer）は状態を持たず送信のみ（失敗時トースト）として扱う。action→コマンド名の変換は
Rust `mapping.rs`（`ir_light_command`）が所有する（詳細は [switchbot-api.md](./switchbot-api.md) 参照）。
...
├── App.tsx                   # 接続状態をロードし connection.saved で AppShell / Onboarding を分岐するルート
...
│   ├── onboarding/           # 未設定時のメニューなし全画面オンボーディング
...
`App.tsx` は起動時に `connection-store.load()` で接続状態を取得し、`connection.saved` を分岐キーにして描画を切り替える（`components/onboarding/onboarding.tsx`）。

- **分岐キーは接続ステータスではなく `connection.saved`**。`!saved`（Token / Secret 未設定）のときサイドバー付き `<AppShell>` の代わりにメニューなし全画面 `<Onboarding>` を描画し、「次の一手（＝接続）」だけに視線を集約する（接続前は各画面が空で操作しても意味がないため）。
- **`saved && disconnected`（設定済みだが到達不能）は通常シェルを維持**する。オンボーディングには入らず、設定画面で再テストする。トレイ表記（未設定「セットアップが必要」／到達不能「オフライン」）は別 Issue で扱う。
- **`!loaded` の間は何も描画しない**（`null`）。分岐が確定する前にオンボーディングを一瞬出すちらつきを防ぐ。
- **接続成功で `connection.saved` が true になると自動でシェルへ遷移**する。オンボーディングは平文 Token / Secret を自身のローカル state にのみ一時保持し、`saveCredentials`（→ 続けて `testConnection`）を呼び終えたらクリアする。追加の遷移コードは持たず、store 更新に伴う `App` の再描画だけで切り替わる。
- 接続ロードは `App` が一元的に行うため、`Sidebar` は `connection.status` を参照するのみで `load()` は呼ばない（重複ロード除去）。
...
- トレイ表示内容は **お気に入りに登録したデバイス / シーンのみ**（補完はしない）。デバイスは種別で絞らず全お気に入りを出し、操作系は `deviceInteraction()` の 3 分岐でウィンドウの `DeviceCard` と同じく出し分ける（`toggle` = Switch、`press` = 「押す」ボタン、`detail` = 右端「>」）。`detail` の「>」はメインウィンドウを前面化しつつ該当デバイスの詳細を開くため `show_main_window({ view: "devices", deviceId })` + `hide_tray_popup` を invoke する。状態サブラベルは `deviceStatusLabel()` を共有し `status` のみを出す（model は前置しない）。フッタ「ウィンドウを開く / 設定 / 終了」は Tauri コマンド（`show_main_window` / `hide_tray_popup` / `quit`）へ結線する。
...
- 最小限の整合として、Rust がメイン show 時に `main-shown` イベントを emit し、メイン側で `device-store` を reload する。トレイの「設定」やデバイス詳細への遷移は `navigate` イベント（Rust emit）→ `navigation-store.navigate(view, deviceId)`。`navigate` の payload は `{ view: string, deviceId?: string }`（Rust 側は `#[serde(rename_all = "camelCase")]` の構造体で emit し、`deviceId` を渡すと該当デバイスの詳細まで遷移する）。show reload はユーザー起動・低頻度でレート 10,000/日を踏まない。
...
OS のログイン項目に登録し、サインイン時にトレイ常駐した状態で自動起動する。公式 `tauri-plugin-autostart`（v2）を採用する。

- Rust `lib.rs` で `#[cfg(desktop)]` ガードのもと `tauri_plugin_autostart::init(MacosLauncher::LaunchAgent, Some(vec!["--hidden"]))` を登録する。これにより autostart 経由の起動には起動引数 `--hidden` が付く（Linux では `~/.config/autostart/<pkg>.desktop` の `Exec` に `--hidden` が入る）。
- **main window は `tauri.conf.json` で `visible:false`** とし、`setup()` 内で `setup_tray(app)?` の後に `std::env::args().any(|a| a == "--hidden")` を判定する。`--hidden` が無い（＝手動起動）ときだけ既存の `commands::show_main` を呼んで main を表示する。表示ロジックは `show_main` に集約し重複させない（SSoT）。
  - **設計判断（論点1）**: 「autostart 判定 → 起動後に main を hide」ではなく「main を最初から不可視にし、必要時だけ show」を採る。前者は一瞬ウィンドウが見えるちらつきが起きうるため、後者（`visible:false` ＋ 明示 show）でちらつきを排除する。手動起動時も初期は不可視だが、`setup()` で即 `show_main` するため実害はない。
  - `setup_tray` は分岐の外で常に実行されるため、autostart / 手動のどちらでもトレイに常駐する。
- 設定 UI は設定画面に新設した「起動」`<section>`（論点2=B）に「ログイン時に起動」トグル（`Switch`）を置く。マウント時に `isAutostartEnabled()` で実状態を反映し、`onCheckedChange` で楽観更新 →`setAutostart(next)`→ 再取得で確定、失敗時は元状態へロールバックする。プラグイン呼び出しは `src/data/autostart.ts` に集約する（境界の単一化。`data/preferences.ts` と同方針）。

### 追加した依存

- Rust: `tauri`（`tray-icon` feature）/ `tauri-plugin-store` / `tauri-plugin-positioner`（`tray-icon` feature）/ `tauri-plugin-single-instance` / `tauri-plugin-autostart`。
- npm: `@tauri-apps/plugin-store` / `@tauri-apps/plugin-autostart`（ポップアップ位置決めは Rust `WindowExt::move_window` で行うため `@tauri-apps/plugin-positioner`（JS）は追加しない）。
- capabilities（`main` / `tray`）: `store:default` / `core:event:default` / `core:window:allow-{show,hide,set-focus}` / `autostart:allow-{enable,disable,is-enabled}`（過不足なく）。
