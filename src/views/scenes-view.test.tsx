...
describe("ScenesView お気に入り（デバイスと同じドロップ先モデル）", () => {
  ...
  it("全件をお気に入りにしても「その他」は残り、ドラッグで解除できる", async () => {
    ...
    const removeZone = await screen.findByRole("list", { name: "その他のシーン" });
    expect(
      within(removeZone).getByText("ここへドラッグするとお気に入りから外れます"),
    ).toBeInTheDocument();

    const favList = screen.getByRole("list", { name: "お気に入り" });
    dragTo(within(favList).getByRole("listitem", { name: /おやすみ/ }), removeZone, 10);

    await waitFor(() => expect(useFavoritesStore.getState().sceneIds).toEqual(["b", "c"]));
  });
